...
const styleSegmentsKey = AttributeKey<List<_LineRendererElement>>('LineRenderer.styleSegments');
...
class LineRenderer<D> extends BaseCartesianRenderer<D> {
  ...
  factory LineRenderer({String rendererId, LineRendererConfig config}) {
    return LineRenderer._internal(rendererId: rendererId ?? 'line', config: config ?? LineRendererConfig());
  }

  LineRenderer._internal({String rendererId, this.config})
      : super(
            rendererId: rendererId, layoutPaintOrder: config.layoutPaintOrder, symbolRenderer: config.symbolRenderer) {
  ...
  /// missing/null data.
  Function _createStackedMeasureOffsetFunction(
    ...
    seriesList.forEach((ImmutableSeries<D> series) {
      ...
      styleSegments.forEach((_LineRendererElement styleSegment) {
        ...
        var animatingElements =
            elementsList.firstWhere((_AnimatedElements elements) => elements.styleKey == styleKey, orElse: () => null);
        ...
        if (animatingElements != null) {
          ...
          final lineAndArea = _createLineAndAreaElements(
              series, styleSegment, stackIndex > 0 ? previousInitialPointList[stackIndex - 1] : null, true);
          ...
          if (_hasMeasureBounds) {
            ...
            for (var index = 0; index < boundsElementList.length; index++) {
              animatingBounds.add(
                  _AnimatedArea<D>(key: boundsElementList[index].styleKey, overlaySeries: series.overlaySeries)
        ...
        final lineAndArea = _createLineAndAreaElements(
            series, styleSegment, stackIndex > 0 ? previousPointList[stackIndex - 1] : null, false);
        final lineElementList = lineAndArea[0];
        final areaElementList = lineAndArea[1];
        final allPointList = lineAndArea[2];
        final boundsElementList = lineAndArea[3];
  ...
  List _createLineAndAreaElements(ImmutableSeries<D> series, _LineRendererElement styleSegment,
  ...
  List<_DatumPoint<D>> _createPointListForSeries(ImmutableSeries<D> series, bool initializeFromZero) {
    ...
    for (var index = 0; index < series.data.length; index++) {
      ...
      pointList.add(
          _getPoint(datum, domainFn(index), series, domainAxis, measure, measureOffset, measureAxis, index: index));
    }
  ...
  ///
  /// [series] the series that this line represents.
  List _createLineAndAreaSegmentsForSeries(List<_DatumPoint<D>> pointList, List<_DatumPoint<D>> previousPointList,
    ...
    if (startPointIndex != null && endPointIndex != null) {
      ...
      if (startPointIndex != endPointIndex) {
        ...
        if (seriesHasMeasureBounds) {
          boundsSegments.add(
              _createBoundsSegment(pointList.sublist(startPointIndex, endPointIndex + 1), series, initializeFromZero));
        }
      }
    }
  ...
  List<_DatumPoint<D>> _createBoundsSegment(
    ...
    areaPointList.addAll(pointList.map((datumPoint) => _DatumPoint.from(
        ...
        initializeFromZero
            ...
            : measureAxis.getLocation(
                series.measureUpperBoundFn(datumPoint.index) + series.measureOffsetFn(datumPoint.index)))));
    ...
    areaPointList.addAll(pointList.reversed.map((datumPoint) => _DatumPoint.from(
        ...
        initializeFromZero
            ? datumPoint.y
            : measureAxis.getLocation(
                series.measureLowerBoundFn(datumPoint.index) + series.measureOffsetFn(datumPoint.index)))));
  ...
  _DatumPoint<D> _getPoint(dynamic datum, D domainValue, ImmutableSeries<D> series, ImmutableAxis<D> domainAxis,
      num measureValue, num measureOffsetValue, ImmutableAxis<num> measureAxis,
      {int index}) {
    ...
    return _DatumPoint<D>(
        datum: datum, domain: domainValue, series: series, x: domainPosition, y: measurePosition, index: index);
  }
  ...
  DatumDetails<D> addPositionToDetailsForSeriesDatum(DatumDetails<D> details, SeriesDatum<D> seriesDatum) {
    ...
    final point = _getPoint(
        seriesDatum.datum, details.domain, series, domainAxis, details.measure, details.measureOffset, measureAxis);
...
class _AnimatedLine<D> {
  ...
  void animateOut() {
    ...
    for (var index = 0; index < newTarget.points.length; index++) {
      ...
      newPoints.add(_DatumPoint<D>.from(targetPoint, targetPoint.x, newTarget.measureAxisPosition.roundToDouble()));
    }
